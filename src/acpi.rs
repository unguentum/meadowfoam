    ...
    fn get_body(&self) -> &[u8] {
        unsafe {
            core::slice::from_raw_parts(
                self.get_body_pointer(),
                self.get_header().length as usize - size_of::<SDTHeader>(),
            )
        }
    }
}
...
impl XSDT {
    ...
    fn find_table(&self, signature: &[u8; 4]) -> Option<*const SDTHeader> {
        unsafe {
            self.get_tables()
                .find(|table| table.as_ref().unwrap().signature == *signature)
        }
    }
    pub fn find_fadt(&self) -> Option<*const FADT> {
        self.find_table(Self::FADT_SIGNATURE)
            .map(|ptr| ptr as *const FADT)
    }
    pub fn get_tables(&self) -> impl Iterator<Item = *const SDTHeader> + '_ {
        (0..self.table_amount()).map(|index| {
            self.get_table(index)
                .expect("Elements in range are present")
        })
    }
    ...
    fn get_table(&self, index: usize) -> Option<*const SDTHeader> {
        ...
        let byte_pointer =
            unsafe { byte_pointer.add(size_of::<SDTHeader>() + index * size_of::<u64>()) };
